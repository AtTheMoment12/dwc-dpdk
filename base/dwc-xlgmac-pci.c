...
static int xlgmac_probe(struct pci_dev *pcidev, const struct pci_device_id *id)
{
	...
	for (i = 0; i <= PCI_STD_RESOURCE_END; i++) {
		if (pci_resource_len(pcidev, i) == 0)//获取bar地址空间的起始地址
			continue;
		ret = pcim_iomap_regions(pcidev, BIT(i), XLGMAC_DRV_NAME);//将bar地址空间映射到内存，供CPU使用
		if (ret)
			return ret;
		break;
	}
...
static struct pci_driver xlgmac_pci_driver = {
	...
	.sriov_configure = xlgmac_sriov_configure,
};
